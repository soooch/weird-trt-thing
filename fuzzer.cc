...
uint8_t sizeof_dtype(nvinfer1::DataType dtype) {
  ...
  case nvinfer1::DataType::kFP8:
    return 1;
  default:
    throw std::invalid_argument("dtype must be variant of DataType");
  }
}

size_t sizeof_tensor(std::unique_ptr<nvinfer1::IExecutionContext> &context,
                     char const *name) {
  ...
  if (format != nvinfer1::TensorFormat::kLINEAR) {
    auto vectorized_dimension =
        context->getEngine().getTensorComponentsPerElement(name);
    auto components_per_element =
        context->getEngine().getTensorComponentsPerElement(name);
    shape.d[vectorized_dimension] +=
        components_per_element -
        (shape.d[vectorized_dimension] % components_per_element);
  }
...
public:
  ...
  void randomize(std::default_random_engine &re) {
    ...
    std::generate((rand_t *)ptr_, (rand_t *)ptr_ + (size_ / sizeof(rand_t)),
                  std::ref(re));
  }
...
struct IOBuffers {
  ...
  void randomize(std::default_random_engine &re) {
    std::for_each(inputs.begin(), inputs.end(),
                  [&re](HostMemory &m) { m.randomize(re); });
...
public:
  ...
  void begin_capture() {
    if (cudaStreamBeginCapture(ptr_, cudaStreamCaptureModeGlobal) !=
        cudaSuccess) {
      throw std::runtime_error("failed to begin capture");
    }
  }

  cudaStream_t inner() { return ptr_; }
...
void try_enqueue(std::unique_ptr<nvinfer1::IExecutionContext> &context,
                 Stream &stream) {
...
int main() {
  ...
  if (!plan0_file.is_open()) {
    throw std::runtime_error("failed to open plan0_path");
  }

  std::ifstream plan1_file(plan1_path);
  if (!plan1_file.is_open()) {
    throw std::runtime_error("failed to open plan1_path");
  }

  // load plan data
  auto plan0 =
      std::vector<char>(std::istreambuf_iterator<char>{plan0_file}, {});
  auto plan1 =
      std::vector<char>(std::istreambuf_iterator<char>{plan1_file}, {});

  // construct engines from plans
  auto engine0_inner =
      runtime->deserializeCudaEngine(plan0.data(), plan0.size());
  if (!engine0_inner) {
    throw std::runtime_error("failed to deserialize plan0");
  }
  ...
  if (!engine1_inner) {
    throw std::runtime_error("failed to deserialize plan1");
  }
  ...
  if (!context0_inner) {
    throw std::runtime_error("failed to create context0 from engine0");
  }
  std::unique_ptr<nvinfer1::IExecutionContext> context0(
      std::move(context0_inner));

  auto context1_inner = engine1->createExecutionContext();
  if (!context1_inner) {
    throw std::runtime_error("failed to create context1 from engine1");
  }
  std::unique_ptr<nvinfer1::IExecutionContext> context1(
      std::move(context1_inner));
